if(!jQuery) {
  throw "jQuery must be included";
}
if(!Pusher) {
  throw "The Pusher JavaScript library must be included";
}
...
function setUp(thermometerSelector) {
  ...
  config.pixelsPerValue = config.middleEl.height()/config.goalValue;
...
function addThermometerMarks(middleEl, numberOfMarks, valuePerMark) {
  for(var i = 1; i <= numberOfMarks; ++i) {
    ...
    markEl.attr('title', '&pound' + amount);
...
function addBehaviours(config, setInitialValues) {
  setInitialValues = (typeof setInitialValues === 'undefined'? true: setInitialValues);
